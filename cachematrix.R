...
makeCacheMatrix <- function(x = matrix()) {
  ...
  set<<-function(B){ #I set the value of the matrix as the matrix is introduce to call makeCacheMatrix
    B<<-X
    inv<<-NULL
  }
  get<-function()X #I get the function and return the value of the matrix which call makeCacheMatrix
...
inv<-X$setinverse() #hte function search if there is an inverse saved in cache memory
  if(!is.null(X)){ #if there is not an inverse matrix in cache memeory then it will pop-up a message indicating is getting cache matrix
